    ...
    for plot, info in land_info.items():
        ...
        for i, name in enumerate(names):
            ...
            if crop_index >= 0 and crop_index < len(crop_info[plot_type]):
                ...
                expected_sales = sales_info[crop_name]['expected_sales']
                
                # 如果产量超过了预期销售量，则超出部分视为滞销
                if crop_production[crop_name] > expected_sales:
                    regular_sales = expected_sales
                    excess_production = crop_production[crop_name] - expected_sales
                    crop_production[crop_name] = regular_sales + excess_production
                    
                    # 计算正常销售部分的利润
                    regular_profit = regular_sales * (sales_info[crop_name]['price'] - sales_info[crop_name]['cost'])
                    
                    # 超出部分不计入利润
                    fitness += regular_profit
                else:
                    # 计算利润
