...
# 文件路径
paths = {
    'crop_types': r'D:\数模\C题\数据预处理\第一题第一问\第一题第一问(dan)\第一题第一问 - 副本\作物种类.xlsx',
    'sales_results': r'D:\数模\C题\数据预处理\第一题第一问\第一题第一问(dan)\第一题第一问 - 副本\连接结果.xlsx',
    'analysis': r'D:\数模\C题\数据预处理\第一题第一问\第一题第一问(dan)\第一题第一问 - 副本\连接结果.xlsx',
    'suitable_plots': r'D:\数模\C题\数据预处理\第一题第一问\第一题第一问(dan)\第一题第一问 - 副本\每种作物适合种植的地块类型与季别_合并.xlsx',
    'output': r'D:\数模\C题\数据预处理\第一题第一问\第一季单季作物.xlsx'
}

# 读取Excel文件
crop_types_df = pd.read_excel(paths['crop_types'], sheet_name='第一季（单）')
sales_results_df = pd.read_excel(paths['sales_results'], sheet_name='2023销售结果')
analysis_df = pd.read_excel(paths['analysis'], sheet_name='销售结果分析')
suitable_plots_df = pd.read_excel(paths['suitable_plots'])
...
#若该地块种植多种作物，则每种作物种植面积不得小于该地块面积的10%
#for year in range(2024, 2031):
#   for plot in areas.keys():
#        for crop in crops:
#          solver.Add(variables.get((year, plot, crop), 0) >= 5)
#          solver.Add(variables.get((year, plot, crop), 0) >= 0.0001 * areas[plot])
...
for year in range(2024, 2031):
    for plot in areas.keys():
        for crop in crops:
           if (year, plot, crop) in variables:
                solver.Add(variables.get((year, plot, crop), 0) >= 0.1 * areas[plot] * binary_vars.get((year, plot, crop), 0))
...
if status == pywraplp.Solver.OPTIMAL:
    ...
    for crop in crops:
        for year in range(2024, 2031):
            total_production = sum([var.solution_value() for key, var in variables.items() if key[0] == year and key[2] == crop])
            expected_sales = crop_info[crop]['产量']
            if total_production > expected_sales:
                excess = total_production - expected_sales
                excess_profit = (crop_info[crop]['价格'] * 0.5 - crop_info[crop]['成本']) * excess
...
with pd.ExcelWriter(paths['output']) as writer:
    results_df.to_excel(writer, sheet_name='第一季单季作物', index=False)
    if excess_results:
        pd.DataFrame(excess_results).to_excel(writer, sheet_name='超出部分收益', index=False)
