...
# 读取Excel文件
sales_results_path = r'C:\Users\HP\Desktop\连接结果(2).xlsx'
crop_types_path = r'C:\Users\HP\Desktop\每种作物适合种植的地块类型与季别_合并.xlsx'
crop_varieties_path = r'C:\Users\HP\Desktop\作物种类.xlsx'

# 读取替代性与互补性数据
substitute_complement_path = r'C:\Users\HP\Desktop\单季作物的替代性与互补性.xlsx'
substitute_complement_df = pd.read_excel(substitute_complement_path)

# 读取相关性数据
correlation_path = r'C:\Users\HP\Desktop\单季作物的产量价格成本相关性.xlsx'
correlation_df = pd.read_excel(correlation_path)

# 处理缺失数据
# 例如，将缺失的相关性值替换为0或其他作物的平均值
correlation_df.fillna(0, inplace=True)  # 这里假设缺失值用0代替，你可以根据实际情况调整
...
# 读取 Excel 文件中的数据
file_path = r"C:\Users\HP\Desktop\作物种类.xlsx"
sheet_name = "第一季（单）"
...
# 将替代性和互补性关系转换为字典
substitute_complement = {}
for index, row in substitute_complement_df.iterrows():
    crop1 = row['作物1']
    crop2 = row['作物2']
    relation = row['相关系数']
    substitute_complement[(crop1, crop2)] = relation

# 将相关性数据转换为字典
correlation_data = {}
for index, row in correlation_df.iterrows():
    crop_name = row['作物名称']
    correlation_data[crop_name] = {
        '产量-销售价格': row['产量-销售价格'],
        '产量-种植成本': row['产量-种植成本'],
        '价格-种植成本': row['价格-种植成本']
    }
...
# 定义需要保留的字段列表
fields_to_keep = ['作物名称', '作物类型', '地块名称', '亩产量/斤', '地块类型', '种植成本/(元/亩)',
...
for index, row in sales_results_df.iterrows():
    ...
    if not crop_info.empty:
        ...
        # 存储到字典中
        actual_planting_2023[(plot_name, crop_name)] = {f'field_{field}': getattr(row, f'field_{field}') for field in
                                                        fields_to_keep}
...
land_areas = {
    ...
    '梯田': {
        f'B{i + 1}': 60 if i == 0 else 46 if i == 1 else 40 if i == 2 else 28 if i == 3 else 25 if i == 4 else 86 if i == 5 else 55 if i == 6 else 44 if i == 7 else 50 if i == 8 else 25 if i == 9 else 60 if i == 10 else 45 if i == 11 else 35 if i == 12 else 20
        for i in range(14)},
...
def monte_carlo_simulation(actual_data, years):
    ...
    for year in range(years):
        ...
        for (plot, crop), data in actual_data.items():
            # 销售量
            if crop in ['小麦', '玉米']:
                sale = data['field_作物产量（销量）'] * (
                            1 + np.random.uniform(sales_growth_rate[crop][0], sales_growth_rate[crop][1]) * year)
            else:
                sale = data['field_作物产量（销量）'] * (
                            1 + np.random.uniform(-sales_growth_rate['其他'], sales_growth_rate['其他']))

            # 亩产量
            yield_per_acre = data['field_亩产量/斤'] * (1 + np.random.uniform(-yield_variation, yield_variation))

            # 种植成本
            cost_per_acre = data['field_种植成本/(元/亩)'] * (1 + cost_growth_rate * year)
            ...
            if data['field_作物类型'] == '粮食类':
                ...
                if crop == '羊肚菌':
                    ...
                    selling_price = data['field_实际销售价格'] * (
                                1 + np.random.uniform(price_growth_rate['食用菌'][0], price_growth_rate['食用菌'][1]))
            else:
                # 处理未分类的作物类型
...
def generate_scenarios(simulated_data, years):
    ...
    for year in range(len(years)):
        ...
        for crop, values in simulated_data[year].items():
            ...
            if crop in ['小麦', '玉米']:
                temp_data[crop] = {
                    'yield_per_acre': values['yield_per_acre'] * (
                                1 + np.random.uniform(sales_growth_rate[crop][0], sales_growth_rate[crop][1])),
                    'selling_price': values['selling_price'] * (
                                1 + np.random.uniform(price_growth_rate['粮食类'], price_growth_rate['粮食类'])),
                    'cost_per_acre': values['cost_per_acre'] * (
                                1 + np.random.uniform(cost_growth_rate, cost_growth_rate)),
                }
            elif crop == '羊肚菌':
                temp_data[crop] = {
                    'yield_per_acre': values['yield_per_acre'] * (
                                1 + np.random.uniform(-yield_variation, yield_variation)),
                    'selling_price': values['selling_price'] * (
                                1 - np.random.uniform(price_growth_rate['羊肚菌'], price_growth_rate['羊肚菌'])),
                    'cost_per_acre': values['cost_per_acre'] * (
                                1 + np.random.uniform(cost_growth_rate, cost_growth_rate)),
                }
            else:
...
def check_constraints(decoded_individual, land_areas, valid_crops, actual_planting_2023):
    ...
    for year in range(len(scenarios)):
        ...
        # 豆类作物三年内种植一次的约束
    legume_years = [year for year in range(len(scenarios)) if
                    any(valid_crops[crop] for crop in valid_crops if decoded_individual[(land_type, plot, crop, year)])]
...
def evaluate(individual, scenarios, land_areas, valid_crops, actual_planting_2023, substitute_complement):
    total_profit = 0
    ...
    for year in range(len(scenarios)):
        for land_type, plots in land_areas.items():
            for plot, area in plots.items():
                for crop1, data1 in scenarios[year].items():
                    for crop2, data2 in scenarios[year].items():
                        if (crop1, crop2) in substitute_complement:
                            relation = substitute_complement[(crop1, crop2)]
                            if decoded_individual[(land_type, plot, crop1, year)] > 0 and decoded_individual[(land_type, plot, crop2, year)] > 0:
                                if relation > 0:  # 互补性
                                    total_profit += relation * data1['yield_per_acre'] * data1['selling_price']
                                else:  # 替代性
                                    total_profit -= abs(relation) * data1['cost_per_acre']
    ...
    for year in range(num_years):
        for land_type, plots in land_areas.items():
            for plot in plots:
                for crop in valid_crops:
                    # 假设每个基因代表作物在地块上的种植比例
                    decoded_individual[(land_type, plot, crop, year)] = individual[index] / len(valid_crops) * \
                                                                        land_areas[land_type][plot]
                    index += 1
    return decoded_individual

# 注册评估函数
toolbox.register("evaluate", functools.partial(evaluate, scenarios=scenarios, land_areas=land_areas, valid_crops=valid_crops,
                 actual_planting_2023=actual_planting_2023, substitute_complement=substitute_complement_df))
...
def evaluate_wrapper(ind):
    return evaluate(ind, scenarios, land_areas, valid_crops, actual_planting_2023, substitute_complement)
...
population, logbook = algorithms.eaSimple(population, toolbox, cxpb=0.5, mutpb=0.2, ngen=NGEN, stats=stats,
...
# 保存结果到Excel
output_path = r'C:\Users\HP\Desktop\种植方案3_1.xlsx'
